...
(async ()=>{
  try{
    ...
    const payload = { ...goal, milestones:[...goal.milestones,{ id:'temp-4', title:'Another milestone', value:0, done:false, isCustom:false }] };
    const patch = await request('PATCH',`/api/goals/${goal.id}`,token,JSON.stringify(payload));
    console.log('PATCH', patch.status); console.log(patch.body);
