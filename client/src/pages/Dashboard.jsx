...
export default function Dashboard() {
    ...
    const scores = useMemo(() => {
        ...
        const calculateScoresForDate = (dateStr) => {
            ...
            const habitsDone = habits.filter(h =>
                h.history && h.history.some(entry => entry.date === dateStr)
            ).length;
    ...
    const data = useMemo(() => {
        const dateRange = (weeklyStats && weeklyStats.length > 0)
            ? weeklyStats.map(s => parseISO(s.date))
            : Array.from({ length: 7 }).map((_, i) => subDays(today, 6 - i));
                ...
                <div>
                    {authLoading || !user ? (
                        <div className="space-y-2 animate-pulse">
                            <div className="h-9 w-48 bg-surface border border-gray-800 rounded-lg"></div>
                            <div className="h-4 w-32 bg-surface border border-gray-800 rounded-lg"></div>
                        </div>
                    ) : (
                        <>
                            <h1 className="text-3xl font-bold text-white">
                                Hello, <span className="text-primary">{user.fullName || user.username}</span> 👋
                            </h1>
